...
def train_model(model, model2, train_loader, val_loader, num_epoch, parameter, patience, device_str):
    ...
    for epoch in range(num_epoch):
        ...
        for batch, data in enumerate(train_loader):
            ...
            if (batch % 100 == 0 and batch != 0):
                ...
                gt_data = (gt[0, 0, :, :]).detach().cpu().numpy()
                filtered_data = median_filter(gt_data, size=3)
                ...
                save_depth(filtered_data, 'tmp/color_gt.png')
            ...
            model2.train()
            optim.zero_grad()
            estimated_depth, confidence = model(rgb, depth)
            estimated_depth2, confidence2 = model2(rgb, depth)
        ...
        # Validation:
        print('Validation')
        val_loss = get_performance_median(model, model2, val_loader, device_str)
        model.to(device)
...
for lr in [1e-3]:
    for wd in [1e-5]:
        ...
        new_model, val_loss, stats = train_model(model, model2, train_loader, val_loader, num_epoch, para_list[0], patience, device_str)
...
print('---------------------------------------------------------------')
print('------------------------ Training Done ------------------------')
save_checkpoint(best_model, 100, "./checkpoints", final_stats)
