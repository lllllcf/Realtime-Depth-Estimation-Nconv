    ...
    for epoch in range(num_epoch):
        ...
        for batch, data in enumerate(train_loader):
            ...
            if (batch % 100 == 0 and batch != 0):
                ...
                save_depth((estimated_depths[0, 0, :, :]).detach().cpu().numpy(), 'tmp_test/color_output.png')
                save_depth((depth[0, 0, :, :]).detach().cpu().numpy(), 'tmp_test/color_sparse.png')
                save_depth((gt[0, 0, :, :]).detach().cpu().numpy(), 'tmp_test/color_gt.png')
            ...
            loss = calculate_loss(estimated_depths, gt)
        ...
        # Validation:
        print('Validation')
        val_loss = get_performance_export(model, val_loader, device_str)
        model.to(device)
