...
def train_model(model, train_loader, val_loader, num_epoch, parameter, patience, device_str):
    ...
    for epoch in range(num_epoch):
        ...
        for batch, data in enumerate(train_loader):
            ...
            model.train()
            optim.zero_grad()
            estimated_depth, _ = model(rgb, depth, k)
...
def get_hyper_parameters():
    _para_list = [{"optim_type": 'adam', 'lr': 1e-3, "weight_decay": 1e-6, "store_img_training": True}]
    _num_epoch = 50
    _patience = 5
    _device = 'cuda'
...
# save checkpoints
save_checkpoint(best_model, 50, "./checkpoints", stats)
# Plotting
...
plt.savefig('./statsNYU.png') # Saves the plot as a .png file