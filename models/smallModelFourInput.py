...
class SMALL_FINAL_all(nn.Module):
...
class SMALL_STEP2(nn.Module):
    ...
    def forward(self, x0_rgb0, x0_d0, x0_rgb1, x0_d1, x0_rgb2, x0_d2, x0_rgb3, x0_d3): 

        x0_rgb = torch.cat((x0_rgb0, x0_rgb1, x0_rgb2, x0_rgb3), dim=0)
        
        # Depth Network
        xout_d = self.step1(x0_d0, x0_d1, x0_d2, x0_d3)
        ...
        return xout[0:1], xout[1:2], xout[2:3], xout[3:4]
...
class SMALL_STEP1(nn.Module):
    ...
    def forward(self, x0_d0, x0_d1, x0_d2, x0_d3): 

        x0_d = torch.cat((x0_d0, x0_d1, x0_d2, x0_d3), dim=0)
